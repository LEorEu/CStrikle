...
- 模型只负责解释、聊天和提交服务器指定落子，不能自行覆盖算法。
- 生产采用 `AI_REASONING_EFFORT=medium`、35 秒单请求上限、SDK 零重试；模型失败后立即使用求解器落子。
...
- 解释指定落子的价值；
- 输出简短中文心理战；
- 调用 `submit_guess` 提交指定人选。

如果模型提交其他选手，工具会拒绝；如果模型超时、429 或调用失败，Room 立即采用同一求解器指定落子，不再重新请求模型或随机猜。
