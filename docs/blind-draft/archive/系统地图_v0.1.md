...
> **状态更新（2026-08-27 傍晚）：** 这份地图写于当天上午，之后原型又走了一版
> （v3.4）并核查了 Major 数据，所以下面几处标记已经变了：
> §2.2 抄底概率 ❓→✅ `12/70/18`；§4.5 Market Days ❓→✅ **7**；
> §8.2 Full Board Reveal 🔵→🟡（已做）；§8.4 情绪标签 🔵→🟡（做了一部分）。
> §8.3 仍是 🔵——改了措辞，但 Decision Quality 那个独立指标没做。
> §10.5 / §15 关于 Major 的排序需要修正，见本节末尾。
> 每条的实测数字见 [原型实测记录.md](原型实测记录.md) 第七、八节。
...
✅ 已定：`12 / 70 / 18`（低估 / 正常 / 高估）。
校准口径不是“板面上有多少张抄底”，而是**到手的五张里有几张**：
25/50/25 时 2 张以上占 58%，改完之后是 0 张 38% / 1 张 41% / 2 张+ 21%。
好货不常打折，坑货倒是不少。
...
- 缺少的位置提高出现权重（`NEED_BOOST = 2.5`）
- **已经满的位置降权**（配额 `{AWPER:1, IGL:1, RIFLER:3}`，超了 ×0.45）——
  软降权不是禁，双狙 / 双指挥仍然摸得到
- 最后阶段才硬保底

只提权不降权时，拿齐 AWP+IGL 之后板面上仍有 **35.6%** 是你不再需要的位置，
五张里两张以上废位的局占 59%——“一批五张”实际上只有三张能选。加降权后是
19.7% / 25%。
...
一度收紧到 6/1，又撤回了——6 轮是在旧发牌器上得出的结论，新发牌器（按预算控价 +
缺位提权 + 后期保底）上没单独验证过，实测第一局就打出反例：跳掉第三天之后第四天
只剩一张步枪，那不是“选哪个步枪”，是“只有这一个还能怎么办”。

UI 不再说“还能跳 N 次”（听着像系统白送刷新），改成 **7 个市场日 · 签 5 个人**。
...
🟡 **蓝图面板（v3.4 已做）**：选人界面常驻一行，显示这一局可以去追哪些
aim 标签、还差几个人（`[DENMARK CORE] 还差 1 个 Denmark 人`）。
零新机制、不加分——aim 标签本来就只用卡面看得见的东西判定，只是把它从赛后
搬到你还能行动的时候。

实测结论：**它没有“制造”构筑意图，但把意图变成了取舍。** 玩家的意图来自认出
gla1ve；而第四天一张丹麦 IGL 同时能点亮 DENMARK CORE 和 TWO CALLERS、代价是
两个指挥抢话 −3，他“犹豫很久”后放弃了——那是 v1 以来第一次有人主动报告犹豫。

❓ `Scouting Focus`（整局一次，下一批把某类人权重 ×2）**先不做**。
它是方向盘，蓝图是目的地；只给方向盘会把玩家导向一个不值得去的地方
（同国三人 +3，还不如一档火力）。等蓝图有了回报再谈。
...
现在的公式是十对两两关系累加、没有上限。它是给“抽出来的五个人”设计的
（0~7 量级），从没在“本来就是一支队”的阵容上跑过，一跑就是：

```
真实 Major 队伍   默契 19.5 ~ 32.0（中位 24.8）   真队友对 10/10
玩家最好那一局    默契  5.0                       真队友对  1/10

同一支玩家队：按基础分排 第 4/33，按总分排 第 17/33
```

不改口径的话，**无论抽得多好都是中游队**，而这正是 Major 之路的前提。
方案和实测见 [比赛引擎_v0.1.md](比赛引擎_v0.1.md) 第 5 节。
...
## 8.3 Hindsight 与 Decision Quality 分离 🔵（只做了一半）
...
上面 §10.5 和这里原本写的是「先别直接做完整 Major」「最后再考虑完整 Major」。
核查数据之后这个判断反过来了：

```
IEM Cologne 2026 / StarLadder Budapest 2025 / BLAST Austin 2025
32 队 · 160 人 · 100% 有卡 · 32/32 队五人齐全
```

对手数据已经躺在库里，**编一个假的 BO1 比用真 Major 更费事**。
「极简」应该体现在**打几个阶段**上，不是对手真不真：第一版只跑一个 16 队的
瑞士轮，跑通了再串三个 Stage。详见 [比赛引擎_v0.1.md](比赛引擎_v0.1.md)。
